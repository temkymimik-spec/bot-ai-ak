...
# Жёсткий критерий (авто-начисление только после реального действия на pocket-option)
SCORE_CONFIRM_THRESHOLD = 0.75      # минимальный скор для начисления
MIN_CONFIRMATIONS = 2              # минимум независимых подтверждений действия
REQUIRE_ACTION = True              # лид без реального действия (депозит/сделка/pocket id) НЕ засчитывается

# Авто-начисление лида
CREDIT_DELAY_DAYS = 1              # сколько дней минимум вести лида перед начислением (не сразу)
